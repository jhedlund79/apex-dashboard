...
const ContribTable = ({ contributions }: { contributions: Contribution[] }) => (
  <div className="bg-surface border border-white/[0.04] rounded-card overflow-hidden mb-6 animate-fadeIn">
    <div className="p-5 pb-0">
      <p className="font-mono text-[12px] uppercase tracking-[1.5px] text-content-muted mb-3.5">Contribution / Deposit History</p>
    </div>
    <Table>
      <thead>
        <tr>
...
const MorganStanley = () => {
  const { data, loading, error, retry } = useMorganStanley()
  ...
  if (!data.connected) {
    return (
      <div>
        <SectionHeader title="◈ Morgan Stanley" subtitle="Investment Portfolio" />
        <PlaidConnect slot="morganstanley" institutionName="Morgan Stanley" onConnected={handleConnected} />
      </div>
    )
  }
