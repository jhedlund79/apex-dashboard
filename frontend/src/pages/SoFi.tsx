          ...
          <tr key={h.ticker} className="hover:bg-white/[0.02] transition-colors">
            ...
            <Td>
              <div className="flex items-center gap-2">
                <div className="h-1.5 bg-brand-blue/20 rounded-full w-20">
                  <div className="h-full bg-brand-blue rounded-full" style={{ width: h.allocation }} />
                </div>
...
const ContribTable = ({ contributions }: { contributions: Contribution[] }) => (
  <div className="bg-surface border border-white/[0.04] rounded-card overflow-hidden mb-6 animate-fadeIn">
    <div className="p-5 pb-0">
      <p className="font-mono text-[12px] uppercase tracking-[1.5px] text-content-muted mb-3.5">Deposit History</p>
    </div>
    <Table>
      <thead>
        <tr>
      ...
      <tbody>
        {contributions.map((c, i) => (
          <tr key={i} className="hover:bg-white/[0.02] transition-colors">
            ...
            <Td className="font-mono text-brand-blue font-semibold">{c.amount}</Td>
            <Td>
              <span className="text-[11px] px-2 py-0.5 rounded-full bg-brand-blue/10 text-brand-blue border border-brand-blue/20 font-mono uppercase tracking-wide">
...
const SoFi = () => {
  const { data, loading, error, retry } = useSoFi()
  ...
  if (!data.connected) {
    return (
      <div>
        <SectionHeader title="◇ SoFi Invest" subtitle="Active Invest Account" />
        <PlaidConnect slot="sofi" institutionName="SoFi" onConnected={handleConnected} />
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <Grid2 className="mb-6">
        ...
        <ChartCard title="Deposit History">
