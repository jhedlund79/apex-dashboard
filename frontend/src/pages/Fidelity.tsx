            ...
            <Td>
              <div className="flex items-center gap-2">
                <div className="h-1.5 bg-accent/20 rounded-full w-20">
                  <div className="h-full bg-accent rounded-full" style={{ width: h.allocation }} />
                </div>
...
const Fidelity = () => {
  const { data, loading, error, retry } = useFidelity()
  ...
  if (!data.connected) {
    return (
      <div>
        <SectionHeader title="◆ Fidelity" subtitle="Brokerage Account" />
        <PlaidConnect slot="fidelity" institutionName="Fidelity" onConnected={handleConnected} />
      </div>
    )
  }
